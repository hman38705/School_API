...
impl AuthController {
    ...
    pub async fn verify_otp_login(
        ...
        // Get user by email
        let user = AuthService::get_user_by_email(&pool, &req.email).await?;

        // Verify OTP
        OtpService::verify_otp(&pool, user.id, &req.otp_code, "login").await?;
    ...
    pub async fn resend_otp(
        ...
        // Resend OTP
        let otp_code = OtpService::resend_otp(&pool, user.id, "login").await?;

        // TODO: Send OTP via email using EmailService
        // For now, just return the OTP (remove in production)
        let response = json!({
